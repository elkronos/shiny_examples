...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("rotation", "Rotation method", c("varimax", "promax", "oblique", "oblimin"))
    ),
...
server <- function(input, output) {
  ...
  output$factor_analysis <- renderPrint({
    if (input$method == "Factor Analysis") {
      fa_results <- fa(data_survey, nfactors = input$n_factors, rotate = input$rotation)
      print(fa_results)
    } else {
      factanal_results <- factanal(data_survey, factors = input$n_factors, scores = c("regression"), rotation = input$rotation)
