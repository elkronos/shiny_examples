...
server <- function(input, output) {
  output$rescue_plot <- renderPlotly({
    ...
    plot_ly(df, x = ~cal_year) %>% 
      add_lines(y = ~Cat, name = "cats", line = list(color = "blue")) %>% 
      add_lines(y = ~Not_Cat, name = "other", line = list(color = "red")) %>% 
      add_ribbons(ymin = ~Not_Cat, ymax = ~Cat, name = "difference", fillcolor = "grey", opacity = 0.3) %>% 
